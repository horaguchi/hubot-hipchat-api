// Derived from https://raw.githubusercontent.com/charltoons/hipchatter/master/hipchatter.js
// Only difference is we return the full response so any adapters can rely on them
...
var Hipchatter = function(token, api_root, proxy) {
    this.token = token;
...
Hipchatter.prototype = {
    ...
    delete_webhook: function(room, id, callback){
        needle.delete(this.url('room/'+room+'/webhook/'+id), null, (this.proxy ? {proxy: this.proxy} : {}), function (error, response, body) {
            // Connection error
    ...
    request: function(type, path, payload, callback){
        ...
        var option = {};
        var jsonOption = {json: true, headers:{'Content-Type': 'application/json; charset=utf-8'}};
        if (this.proxy) {
            option.proxy = this.proxy;
            jsonOption.proxy = this.proxy;
        }

        // GET request
        if (type.toLowerCase() === 'get') {
            var url = payload.hasOwnProperty('token') ? this.url(path, payload, payload.token) : this.url(path, payload);
            needle.get(url, option, requestCallback);

        // POST request
        } else if (type.toLowerCase() === 'post') {
            var url = payload.hasOwnProperty('token') ? this.url(path, payload.token) : this.url(path);

            needle.post(url, payload, jsonOption, requestCallback);

        // PUT request
        } else if (type.toLowerCase() === 'put') {
            needle.put(this.url(path), payload, jsonOption, requestCallback);

        // DELETE request
        } else if (type.toLowerCase() === 'delete') {
            needle.delete(this.url(path), null, option, requestCallback);
